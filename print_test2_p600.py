...
for color in allColors:
    raster1 += ESC_i_matrix(color, vec, spacing=0, fan=0)*raster1_repetitions


# raster2
raster2_repetitions = 1
raster2 = b''
x = 1
y = 2
dy = 0.25
raster2 += ESC_v(pmgmt, y)  # move 2 inch away from top pattern
for color in allColors:
    raster2 += ESC_v(pmgmt, dy) + ESC_dollar(hor, x) + \
        ESC_i_matrix(color, vec, spacing=0, fan=0)*raster2_repetitions

# put all individual rasters together.
rasterdata = raster1 + raster2 + b'\x0c'
